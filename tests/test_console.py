...
class TestHBNBCommand(unittest.TestCase):
    ...
    def test_create_state_city(self):
        """Test create State name="California" + create City state_id="<new state ID>" name=San_Francisco"""
        with patch("sys.stdin", StringIO("create State name=\"California\"\ncreate City state_id=\"<new state ID>\" name=\"San_Francisco\"\nquit\n")):
            self.assert_stdout("(hbnb) \n(hbnb) \n(hbnb) \n", self.console.cmdloop)

    def test_create_state_multiple_cities(self):
        """Test create State name="California" + create City state_id="<new state ID>" name=Fremont"""
    ...
    def test_create_state_city_user_place(self):
        ...
        commands = [
            ...
            "create City state_id=\"<new state ID>\" name=\"San_Francisco_is_super_cool\"\n",
            "create User email=\"my@me.com\" password=\"pwd\" first_name=\"FN\" last_name=\"LN\"\n",
            "create Place city_id=\"<new city ID>\" user_id=\"<new user ID>\" name=\"My_house\" description=\"no_description_yet\" number_rooms=4 number_bathrooms=1 max_guest=3 price_by_night=100 latitude=120.12 longitude=101.4\n",
