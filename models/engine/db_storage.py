...
class DBStorage:
    ...
    def all(self, cls=None):
        queryClass = [User, City, Place, State, Amenity, Review]
        queryList_Output = []
        queryDict_Output = {}
        if cls in queryClass:
            queryList_Output = session.query(cls).all()
        else:
            queryAll = session.query(queryClass).all()
            for class_Input in queryClass:
                queryList_Output +=  session.query(class_Input).all()
        for class_Output in queryList_Output:
            queryDict_Output.update({class_Output.__class__.__name__ + "." + class_Output.id: class_Output})
