...
# Define a teardown function to close the SQLAlchemy session after each request.
...
def display_states_and_cities(id=None):
    ...
    if not id:
        # Prepare a dictionary for rendering the list of states and their names.
        states_dict = {state.id: state.name for state in all_states.values()}
        
        # Render the "7-states_list.html" template to display the list of states.
    ...
    if state_key in all_states:
        ...
        return render_template("9-states.html",
                               Table="State: {}".format(all_states[state_key].name),
                               items=all_states[state_key])

    # If the state is not found, render the same template to display "Not found!"
