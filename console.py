...
class HBNBCommand(cmd.Cmd):
    ...
    def precmd(self, line):
        ...
        try:  # parse line left to right
            ...
            # isolate _id, stripping quotes
                ...
                args_start = pline.find(',') + 1
                args_end = pline.rfind(')')
                args_str = pline[args_start:args_end].strip()

                if args_str.startswith('{') and args_str.endswith('}'):
                    # If it looks like a dictionary, treat it as kwargs
                    _args = args_str
                else:
                    # If not, treat it as regular args
                    _args = args_str.replace(',', '').strip()
    ...
    def do_create(self, args):
        ...
        for param in args[1:]:
            # Split the parameter into key and value using '=' as separator
        ...
        # Remove double quotes if the value is enclosed in them
            ...
            # Try to convert the value to its appropriate data type
            try:
                if '.' in value:
                    # Float
                    value = float(value)
        ...
        # Save the instance to the storage engine
        new_instance.save()
