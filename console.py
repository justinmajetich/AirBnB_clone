...
class HBNBCommand(cmd.Cmd):
    ...
    def do_create(self, args):
        ...
        attrsIgnored = ('id', 'created_at', 'updated_at', '__class__')
        class_name = ''
        name = r'(?P<name>(?:[a-zA-Z]|_)(?:[a-zA-Z]|\d|_)*)'
        class_match = re.match(name, args)
        obj_kwargs = {}
        if class_match is not None:
            ...
            string = r'(?P<t_str>"([^"]|\")*")'
            floats = r'(?P<t_float>[-+]?\d+\.\d+)'
            integer= r'(?P<t_int>[-+]?\d+)'
            parameter = '{}=({}|{}|{})'.format( name, string, floats, integer)
            for param in params:
                param_match = re.fullmatch(parameter, param)
